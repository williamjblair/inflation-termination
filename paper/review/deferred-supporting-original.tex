...
This appendix records four quantitative results that accompany the main text. Nothing in the body of the paper depends on them. Each fixes a constant, an exact order, or a ceiling that would otherwise be guessed, and each carries its own verification status, stated at the end of the item.
...
The family below improves the $\Omega(n^{-3})$ rate recorded for $H_n$ in Section~\ref{sec:general} along the family of Section~\ref{sec:main}. Its distance bound uses the parity rigidity of compatible laws rather than a Finner violation; every atomic Finner inequality holds for it.
...
The convergence bound of Theorem~\ref{thm:convexorder} extracts a compatible law from a feasible table by sampling source indices independently. Requiring the extracted law to come from a table in the support of the given witness loses nothing on the parity face, from order six on. At order two there is an exact obstruction.
