...
\end{equation}
For rational $\eps$, these entries are rational. Here $F_t$ is the map \eqref{eq:outputs} from defect configurations to observed assignments; they are nonnegative and sum to $(\eps+1-\eps)^{t^3}=1$.

The defect cube is not a triangle model, and $\Gamma_t$ does not come from running an inflated triangle model; it is a probability law on the copied observations, which is what a finite inflation test asks for.
