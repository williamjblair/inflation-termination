...
\]
for every $t$ and every atom. The obstruction therefore does not come from vanishing probabilities. The distance to $\Cpath$ shrinks at rate $t^{-2}$, faster than the rate $t^{-1/2}$ that Theorem~\ref{thm:convexorder} forces; whether the path carries survivors at distance of order $t^{-1}$, as the square and the triangle do, is not settled here.
