...
A path is a tree, and trees carry no parity obstruction of the kind used in Section~\ref{sec:cycles}. The five-observer path nonetheless admits, at every order, a strictly positive law that the strongest of the three tests accepts and that no model produces. The scenario, in the language of Section~\ref{sec:pairsource}, is
\[
A(X),\qquad B(X,L),\qquad C(L,R),\qquad D(R,Z),\qquad E(Z),
\]
with $X,L,R,Z$ mutually independent. We write $\Cpath$ for its compatible set, record the endpoint observations $A,E$ as bits $x,z$ and the middle observations $B,C,D$ as signs. The order-$t$ inflation of Definition~\ref{def:ginflation} has copies $X_a,L_i,R_j,Z_e$, $a,i,j,e\in[t]$, and the $3t^2+2t$ copied observations
\[
...
\]
with a strictly positive rational witness. Consequently $\tmin^H(P_{h_t})>t$ for $H\in\{\mathrm{NW},\mathrm{AI},\mathrm{exp}\}$, no finite order of any of the three hierarchies characterizes $\Cpath$, and
\[
...
E^e=Z_e .
\end{equation}
Each copied observation is a function of the data attached to its copied parents together with its own private sign: $r_i$ and $u_i$ belong to $L_i$, and $s_j$ and $v_j$ to $R_j$. Call the resulting law $\Gamma_t$; by \eqref{eq:Htpos} it is strictly positive and rational. As in Section~\ref{sec:construction}, $\Gamma_t$ is a law on the copied observations and not the output of an inflated model: under $H_t$ the families $u$ and $v$ are correlated.

\emph{Formal weights.} Give each $u_i$ the complex weight $(1+i\gamma u_i)/2$ and each $v_j$ the weight $(1-i\gamma v_j)/2$, and keep the actual probabilities of $X,Z,r,s,\nu$. Write $\E_\ast$ for the resulting formal expectation, a product functional in which all $2t+2t+t^2+2t$ coordinate families are independent, with $\E_\ast u_i=i\gamma$ and $\E_\ast v_j=-i\gamma$. Since the weights of the auxiliary coordinates multiply to a complex number whose real part is $H_t$, and the other coordinates carry real weights,
\begin{equation}\label{eq:realpart}
\E_{\Gamma_t}[F]=\Real\,\E_\ast[F]\qquad\text{for every real-valued }F .
\end{equation}
The complex weights are a device for computing marginals; the witness is the real law $\Gamma_t$.
...
\emph{Injectable marginals.} An injectable set of the path contains at most one copied observation over each of $A,B,C,D,E$ and its copy indices agree on shared sources, so it is contained in a copied path $(a,i,j,e)$ with consistent indices, completing unused indices arbitrarily. Its marginal is therefore the corresponding marginal of $P_h$, and $\E_\ast[G]$ is real for every function $G$ of it.
...
\]
for every $t$ and every atom. The obstruction is therefore not carried by vanishing probabilities. The distance to $\Cpath$ shrinks at rate $t^{-2}$, faster than the rate $t^{-1/2}$ that Theorem~\ref{thm:convexorder} forces; whether the path carries survivors at distance of order $t^{-1}$, as the square and the triangle do, is not settled here.
