...
\]
We recall the precise finite de Finetti estimate behind this in the proof of Proposition~\ref{prop:promised}.
...
The lower bound is the passing construction of Section~\ref{sec:main} and the upper bound is the first fan inequality of Theorem~\ref{thm:fan}; they agree in exponent and not in constant. Section~\ref{sec:distance} proves the distance statements for the same family.
...
The gap between the constants $\tfrac12$ and $4-2\sqrt3$ is a gap between the passing construction and the fan certificate; whether $\eps^{1/3}\tmin^H(P_\eps)$ converges, and to what, is open. Exact certificates at $\eps=k^{-3}$ show that the two bounds coincide for $k\in\{4,6,8,10,16\}$ (first rejecting order $4,5,6,7,10$) and differ at the tested values $k\ge32$ (Section~\ref{sec:repro}).
