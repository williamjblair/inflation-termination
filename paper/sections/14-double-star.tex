...
\]
the largest leaf alphabet size, or $2$ if all leaf alphabets are smaller.
...
\end{equation}
the conditioning event on the right having positive probability whenever every $x_s^{\,i_s}$ and $z_j^{\,k_j}$ is supported. This identity is the finite-order step: it is a statement about the witness, derived from symmetry and the diagonal law alone.
...
The construction samples new independent sources; it does not assume that conditioning on $E$ preserves any independence among the latent variables of a hypothetical model behind $\Gamma$. The only fact used about $\Gamma$ is the identity \eqref{eq:condmarg}. A central source with
\(
\prod_s|\mathcal O_b|^{|\supp P_{\ell_s}|}\cdot\prod_j|\mathcal O_c|^{|\supp P_{r_j}|}
\)
values suffices, since that is the number of pairs of response tables.
...
A star is the case $q=0$, with $c$ one of its leaves. There the bound of Theorem~\ref{thm:doublestar} is not the best available: order two suffices with arbitrary finite alphabets \cite[Section 4.1]{NW2020}, by taking independent sources equal to the leaf values and letting the centre respond with its conditional law given them. Theorem~\ref{thm:doublestar} adds the central edge, where the two centres share a source and the leaf values on the two sides must be assembled into one pair of response tables.
