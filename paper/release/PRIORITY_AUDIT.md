...
## Addendum 2026-09-16: Section 6.3, the removed Section 7, and Fritz 2012

Source: the sweep `research/2026-09-16/priority/PRIORITY_SWEEP_SECTIONS_6.3_7.md` in the author's development repository (primary full texts from arXiv, bibliographic data checked against arXiv and Crossref), run against commit `0885897`. After it, Section 6.3 was cut to Remark 6.3 and Section 7 (sources shared by three or more observers) was removed from this manuscript for a companion paper. The rows below record the verdicts that bear on what remains, and the attribution now in the text. The verdicts for the removed hypergraph results (absorption, traces, observers reading every source, hyper-double-star reconstruction, pair-set nontermination, the open case `K_4^(3)`) belong to the companion paper and are kept in the sweep.

| result | verdict | strongest predecessor | action taken in the manuscript |
|---|---|---|---|
| Remark 6.3, cubic parity certificates (square constant 8, triangle constant 10) | PARTIALLY_OVERLAPPING in form; NO_MATCH for the specific certificates | Wolfe–Spekkens–Fritz 2019, Section IV C, eqs. (51)–(54) of arXiv v5: cubic triangle inequalities in `E[A]`, `E[B]`, `E[C]`, `E[ABC]` from order-two inflation; the new certificates are not among them (835 of 7152 sampled laws with `F < 0` satisfied all 52 of WSF's inequalities, a sampled numerical check) | WSF cited at the remark; the certificates are described as adding the errors of the parity-rigidity argument of Boreiri et al. 2023 and 2025 |
| Remark 6.3, `F(P) >= -Σ(P)/n` and the rejecting order `⌊Σ(P)/(-F(P))⌋ + 1` | SUBSTANTIALLY_PRECEDED in mechanism and rate; NO_MATCH for the source-free constant | Navascués–Wolfe 2020, Theorem 1 with eqs. (34) and (A5): a degree-`g` polynomial inequality valid on `C` holds on `I_n^NW` up to `O(Lg²/n)`, with a constant that depends on the number of sources | the remark attributes the `1/n` mechanism to NW and states that the new point is the source-free constant `Σ(P) <= 3`; the former quadratic part (ii) was removed |
| former Proposition 6.6 (laws with `F < 0` satisfy every Finner inequality) | PARTIALLY_OVERLAPPING | Navascués–Wolfe 2020 eqs. (30)–(31) (order two beats Finner on `P_{q,r}`); Renou et al. 2019 footnote 4 (W satisfies the Finner inequality) | proposition removed; the remark keeps one sentence on the W law with both citations |
| former Remark 6.7 (worked examples) | CLEARLY_PRECEDED for the W law; SUBSTANTIALLY_PRECEDED as a whole (all three laws violate WSF eq. (52), so they are rejected at AI order two) | Wolfe–Spekkens–Fritz 2016/2019, Example 2 (W incompatibility); Navascués–Wolfe 2020 footnote 2 (W rejected by NW at order three) | examples removed; the remark cites WSF Example 2 and NW footnote 2 and says the order 73 it gives for W is far from tight |
| Remark 3.9 (one source read by three observers: every law compatible) | CLEARLY_PRECEDED | Fritz 2012, Proposition 3.7 | cited at the remark |
| Lemma 3.7 (exhaustion) and the assembly of Theorem 3.2 | PARTIALLY_OVERLAPPING in method; the classification itself is not preceded | Fritz 2012, Theorem 3.8 and Lemma 3.9: graph scenarios with only classical correlations are the star forests, and every other connected graph has an induced `C_3`, `C_4` or `P_4`, with extension from the induced subgraph by deterministic outcomes; Fritz 2012 Section 3 also frames general scenarios as hypergraphs | a paragraph after the proof of Lemma 3.7 names Fritz's theorem as the direct ancestor of the forbidden-induced-subgraph shape of the argument and of the hypergraph framing; this closes a gap in the 2026-09-14 sweep, which did not record it |

`PRIORITY_STATUS (Remark 6.3, Remark 3.9, Lemma 3.7) = PRIORITY_NOT_KILLED. The 1/n mechanism of Remark 6.3 is Navascués–Wolfe's and is attributed; its source-free constant and the two certificates have no located predecessor. The W law and its Finner compliance are preceded and cited. Fritz 2012 Theorem 3.8 and Lemma 3.9 precede the shape of the exhaustion argument and are cited; no predecessor was located for the pair-source classification beyond stars. Firstness not certified.`
