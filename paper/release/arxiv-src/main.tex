...
\node[font=\small,align=center] at (1.9,-1.5) {Double-stars\\diameter $\le3$};
\node[font=\small,inkgreen] at (1.9,-2.25) {$\mathcal I_2^H(G)=\mathcal C_G$};
\draw[black!25] (4.7,1.8)--(4.7,-2.55);
...
\node[font=\small] at (6.3,-1.5) {Induced cycle $C_m$};
\foreach \i in {0,...,4}{\node[observer,minimum size=3mm] (p\i) at (8.3+0.62*\i,0) {};}
\draw[network] (p0)--(p1)--(p2)--(p3)--(p4);
\node[font=\small] at (9.55,-1.5) {Induced path $P_5$};
\node[font=\small,inkrust] at (8.2,-2.25) {$\mathcal C_G\subsetneq\mathcal I_t^H(G)\quad\text{for every }t$};
\end{tikzpicture}
...
The triangle scenario (Figure~\ref{fig:triangle}) has three mutually independent latent variables $X,Y,Z$ take values in arbitrary measurable spaces with laws $\mu_X,\mu_Y,\mu_Z$. Three observed variables with finite alphabets are generated by stochastic response kernels,
\[
...
\node[observer] (A\suffix) at (\off+0.3,-.5) {\ifnum\off=0 $A$\else $A^{ij}$\fi};
\node[observer] (B\suffix) at (\off+1.8,-.5) {\ifnum\off=0 $B$\else $B^{ik}$\fi};
\node[observer] (C\suffix) at (\off+3.3,-.5) {\ifnum\off=0 $C$\else $C^{jk}$\fi};
...
\node[font=\small] at (1.8,-1.2) {Original scenario};
\node[font=\small] at (8.3,-1.2) {One copied triangle $\Delta_{ijk}$};
...
We extend the triangle definitions to pair-source graphs and prove the structural lemmas needed for the classification.
...
Each source is read by exactly the two endpoints of its edge. The triangle of Section~\ref{sec:setup} is the scenario of the $3$-cycle, $\Ctri=\Cscen{C_3}$; the graph $P_n$ is the path on $n$ vertices.
...
In the inflated graph every source copy is a root and every copied observation is a sink, which makes $d$-separation explicit.
...
\section{The five-observer path}\label{sec:fivepath}
...
\subsection{The target}\label{sec:pathtarget}
...
\subsection{A bilocal obstruction}\label{sec:bilocal}
...
\subsection{The witness at every order}\label{sec:pathwitness}
...
\section{Finite-order fan inequalities}\label{sec:fan}
...
The parameter $\eps$ of the family $P_\eps$ is, to leading order, a fixed multiple of the distance from $P_\eps$ to the compatible set, in total variation and in Euclidean norm. With Proposition~\ref{prop:family} the exponent becomes a statement about the geometry near the boundary of $\Ctri$.
...
\section{Rational input bit-length complexity}\label{sec:bits}
...
\begin{table}[htbp]
\centering\small
\begin{tabular}{@{}p{.21\linewidth}p{.20\linewidth}p{.51\linewidth}@{}}
\toprule
...
The manuscript sources, figure-generation script, exact certificates and Lean development are available at \repo. Section~\ref{sec:repro} gives the verification commands and formalization boundaries. The binary, finite-latent classification is registered as \texttt{PALOMAR-2026-09-14-000009}, version~1.
