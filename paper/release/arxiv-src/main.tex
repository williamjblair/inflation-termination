...
The proof reconstructs a model on each double-star. Every other connected graph contains an induced cycle or a five-vertex path; explicit witnesses on these subgraphs extend to the ambient graph. Figure~\ref{fig:classification} summarizes the classification.
...
We omit the graph argument from $\INW_t$, $\IAI_t$ and $\Iexp_t$ when discussing the triangle. Definition~\ref{def:NW} is the full-table symmetric formulation of \cite[equations (16)--(19)]{NW2020}; its lower-degree diagonal conditions follow by marginalization. Their cyclic indexing and ours describe the same copied variables. The injectable sets are exactly the subsets of copied triangles (Appendix~\ref{app:injectable}). The AI conditions also prescribe products for sets with disjoint copied ancestors, for example
\[
\Law(A^{12},B^{21},C^{12})=P_A\otimes P_B\otimes P_C.
\]
The NW test does not impose this relation at order two.

Every compatible triangle law satisfies the event form of Finner's inequality,
...
For every cycle and every order, we construct a strictly positive law that passes all three tests and has positive distance from compatibility.
...
For the five-observer path we use a bilocal inequality in place of the cycle parity obstruction. The scenario is
\[
...
\]
for every $t$ and every atom. The theorem gives a distance lower bound of order $t^{-2}$ and leaves the distance asymptotics of this family undetermined. The general upper bound is $O(t^{-1/2})$. It remains open whether the path admits accepted laws at distance $\Omega(t^{-1})$, as the triangle and square do.


\section{Quantitative convergence on the triangle and square}\label{sec:cyclequant}

We bound the distance from finite-order feasible laws to compatibility on the triangle and square. Modifying the unconstrained Fourier coefficients of the parity witness improves the lower bound from $\Omega(t^{-2})$ to $\Omega(t^{-1})$. A convex-order argument gives an $O(t^{-1/2})$ upper bound. We then compare the hierarchy orders and certify finite threshold gaps.
...
R:=(1+q)^{t/2}\le\frac{m^2}{m^2-1},\qquad\text{equivalently}\qquad
q\le\begin{cases}
(9/8)^{2/t}-1,&m=3,\\
(16/15)^{2/t}-1,&m=4.
\end{cases}
...
At order ten, Table~\ref{tab:thresholds} gives $tq_t^H\approx0.385$ (AI) and $0.401$ (NW) on the square, and $0.833$ (AI) and $0.870$ (NW) on the triangle. These exceed the asymptotic constants $2\ln(16/15)\approx0.129$ and $2\ln(9/8)\approx0.236$ in our all-order feasibility guarantees. The finite brackets do not determine whether $tq_t^H$ converges. Exact certificates extend the odd and even pattern of Proposition~\ref{prop:paritythresholds} on the triangle: $q^{\mathrm{AI}}_t$ and $q^{\mathrm{NW}}_t$ have identical certified brackets at $t=11$ and $t=13$, with an ancestral-independence dual supported on Navascu\'es--Wolfe keys only, and $q^{\mathrm{AI}}_{12}<q^{\mathrm{NW}}_{12}$. As in Proposition~\ref{prop:paritythresholds}(ii), identical brackets do not establish equal thresholds. The odd/even pattern remains open beyond the certified orders.
...
The parity laws of Sections~\ref{sec:cycles} and~\ref{sec:cyclequant} give nontermination and worst-case lower bounds. This section studies a second triangle family, whose passing and rejecting bounds have the same exponent and whose distance to compatibility we compute to leading order.
...
Theorem~\ref{thm:main} includes $t=1$, where $P_1=\tfrac12(\delta_{000}+\delta_{111})$ is the perfectly correlated three-bit law, the standard example of a triangle-incompatible distribution, with Finner margin $1/8=\eps_1^2/2$; it passes the trivial order-one test.
...
$\Gamma_t$ is a probability law on the copied observations, which is what a finite inflation test asks for; we do not obtain it by running an inflated triangle model.
...
Every $P\in\Ctri$ satisfies $P(000)^2\le P_A(0)P_B(0)P_C(0)$.
\end{lemma}
\begin{proof}
...
We compute the leading distance from $P_\eps$ to $\Ctri$, which expresses the rejecting-order exponent in terms of distance.
...
\emph{Lower bounds.} The product law $\Bern(1-\sigma)^{\otimes3}$ is compatible and within total variation $\eps$ of $P_\eps$. Compactness of $\Ctri$ gives a nearest law in either norm \cite{RGW2018}. A total-variation minimizer $Q$ has distance $\delta=O(\eps)$. Since probabilities of events differ by at most $\delta$,
\[
...
Corollary~\ref{cor:geometry} follows from Theorem~\ref{thm:distance} and Proposition~\ref{prop:family}. The constant $1-2^{-3/2}$ is the fraction of the rare atom $P_\eps(000)\approx\eps$ that, by the Finner inequality, no compatible law with zero marginals near $\tfrac12\eps^{2/3}$ can retain.
...
Can one obtain computable rejecting-order bounds, with explicit constants, for every incompatible rational law on the triangle or square, including laws that satisfy all Finner and cubic parity inequalities considered here?
\end{question}
...
The binary, finite-latent classification is registered as \texttt{PALOMAR-2026-09-14-000009}, version~2, at the \href{https://palomar-registry.org/entry.html?id=PALOMAR-2026-09-14-000009&version=2}{Palomar registry}. That registration concerns the stated classification theorem, not all results in this paper.
...
The right inclusion is Proposition~\ref{prop:gsound} and the equality is Lemma~\ref{lem:rootsink}. For the left inclusion let $\Gamma$ be an order-$n$ Navascu\'es--Wolfe witness for $P$ with $n=\lfloor3t/2\rfloor$ and let $\Gamma'$ be its restriction to the copy indices $1,\dots,t$ of each source. Restriction preserves symmetry and the diagonal law of degree $t$, so $\Gamma'$ witnesses $P\in\INW_t$; we check the AI prescriptions.
...
The factor $3/2$ comes from placing all components on diagonal rows at once; ancestrally disjoint collections of $\lfloor3t/2\rfloor$ injectable components whose source-type conflict graph is complete show that this method cannot do better.
...
Let $U_{ijk}$, $(i,j,k)\in[n]^3$, be independent with a common law $\mu$ on a finite alphabet, let each observed type $V\in\{A,B,C\}$ carry a permutation-invariant kernel $g_V$ giving its probability of output one from the $n$ roots on its line, and let the responses be independently randomized across copied observations. The resulting table is a full order-$n$ witness, and with the profiles $f_V(s)=\E[g_V(s,U_2,\dots,U_n)]$ and $m_V=\sum_s\mu_sf_V(s)$ the target is $\sum_s\mu_s\bigotimes_V\Bern(f_V(s))$. Write $\mathcal L_n^{(\kappa)}$ for the laws obtained this way from a root alphabet of size $\kappa$.
...
\begin{proof}
Write $f=f_V$, $m=m_V$, $h=f-m$, $v=\E h(U)^2$ and $T=\sum_{i=1}^nh(U_i)$, and let $g=g_V(U_1,\dots,U_n)$. Independence gives $\E T^2=nv$, and symmetry of the kernel gives $\E[(g-m)T]=n\E[h(U_1)(g-m)]=nv$. Cauchy--Schwarz then gives $(nv)^2\le\operatorname{Var}(g)\,nv$. For $v>0$ divide by $nv$ and use $0\le g\le1$ to get $nv\le\operatorname{Var}(g)\le m(1-m)$; the case $v=0$ is immediate.
\end{proof}
