...
We answer this question for pair-source scenarios, in which each source reaches exactly two observers. Such a scenario is a finite simple graph $G$: observers are vertices and sources are edges. A companion paper treats scenarios with sources shared by three or more observers. We then study the order needed to reject incompatible laws near the compatible set.
...
Let three observers read one common source. That source can carry a sample of the whole observed triple, so every law on the three outcomes is compatible~\cite[Proposition 3.7]{Fritz2012} and the order-one test characterizes the compatible set. The observed adjacency graph of this scenario is the triangle, which does not terminate at any order. Once a source may be read by three or more observers, the adjacency graph alone does not decide termination. A companion paper treats such scenarios.
\end{remark}
...
\end{question}
Proposition~\ref{prop:paritythresholds} gives finite-order separations. It remains open whether $\IAI_t\subsetneq\INW_t$ holds at every order on any fixed scenario.
...
The files below are under \texttt{artifact/certificates/} in the repository \repo, with the checkers under \texttt{artifact/verifiers/} and the manifest of SHA-256 identities at \texttt{artifact/MANIFEST.json}.
...
Theorem~\ref{thm:identitytransfer} does not extend from identities to inequalities, and it fails already at degree two. Let $n$ be even and take the table $A^{ij}=b_i$ with half the $b_i$ equal to zero and half to one, and $B,C$ constant. The polynomial $f(q)=(q_A(1)-\tfrac12)^2$ is nonnegative on every reweighting, while its degree-two polarization on distinct source indices is
\[
