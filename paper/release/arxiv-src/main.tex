...
\end{tikzpicture}
\caption{The five-observer path. Conditioning on $A=x$ and $E=z$ changes only the outer sources $X,Z$; the inner sources $L,R$ remain independent, which gives the bilocal inequality. The target violates it by $\sqrt{1+h}-1$ and has full support.}\label{fig:path}
\end{figure}
...
$D_{ijk}=0$: the remaining inputs are disjoint, so the three outputs are independent $\Bern(r)$.\par\smallskip
