...
The argument has a direct ancestor. Fritz classified the graph scenarios in which every correlation is classical as the disjoint unions of stars~\cite[Theorem 3.8]{Fritz2012}: every other connected graph has an induced triangle, square or four-vertex path~\cite[Lemma 3.9]{Fritz2012}, and a correlation on that induced subgraph extends to the whole graph by deterministic outcomes at the remaining vertices. Lemma~\ref{lem:exhaustion} and the proof below have the same shape, with double-stars in place of stars, cycles and the five-vertex path as the forbidden induced subgraphs, and inflation witnesses in place of correlations. The framing of scenarios with arbitrary sources as hypergraphs is also Fritz's~\cite[Section 3]{Fritz2012}.
...
Table~\ref{tab:thresholds} shows two further regularities that the theorems do not explain. The products $tq^H_t$ increase towards constants near $0.4$ on the square and $0.9$ on the triangle, about three and four times the constants $2\ln(16/15)\approx0.129$ and $2\ln(9/8)\approx0.236$ of Theorems~\ref{thm:squarelinear} and~\ref{thm:trianglelinear}; and the two hierarchies have identical certified brackets along this direction at every odd order up to nine, while they separate at every even order. Exact certificates extend the second pattern on the triangle: $q^{\mathrm{AI}}_t$ and $q^{\mathrm{NW}}_t$ have identical certified brackets at $t=11$ and $t=13$, with an ancestral-independence dual supported on Navascu\'es--Wolfe keys only, and $q^{\mathrm{AI}}_{12}<q^{\mathrm{NW}}_{12}$. As in Proposition~\ref{prop:paritythresholds}(ii), identical brackets do not establish equal thresholds. Neither pattern is proved beyond the certified orders.
...
That a polynomial inequality valid on the compatible set holds on $\INW_n$ up to an error of order $1/n$ is Theorem~1 of Navascués and Wolfe with their equation~(34), and their equation~(A5) makes the constant explicit~\cite{NW2020}; that constant depends on the number of sources. The new point is the source-free constant $\Sigma(P)\le3$. Cubic triangle inequalities of the same form, in $\E A$, $\E B$, $\E C$ and $\E[ABC]$, appear among the order-two inequalities of Wolfe, Spekkens and Fritz~\cite[Section IV C, eqs.~(51)--(54) of arXiv v5]{WSF2019}; the certificates above are not among them. A certificate can reject a law that satisfies the Finner inequality \eqref{eq:finner} at every atom. The W law, uniform on the three outcomes with exactly one $1$, has $-\E A\,\E B\,\E C+10\,P(ABC=1)=-1/27$ and $\Sigma=8/3$, so the bound gives $\tmin^H\le73$. Its incompatibility with the triangle is Example~2 of~\cite{WSF2019}, and that it satisfies the Finner inequality is noted in~\cite[footnote 4]{Renou2019}. Navascués and Wolfe report that their hierarchy rejects it at order three~\cite[footnote 2]{NW2020}, so the order above is far from tight on this law.
\end{remark}
